...
public class XueKeJSServlet extends HttpServlet {
		...
		if (index != null) {
			if (flag == null) {
				if (rank != null) {
					...
					List<Map<String, String>> list = xkjss.queryScore(rank,
							authorseq, userid);
					w.write(list.get(0).get("yuan")+","+list.get(0).get("xiao"));
				} else {
					String zicd = "学科建设";
					List rankList = xkjss.queryRank(userid, zicd);
					session.setAttribute("rankList", rankList);
					req.getRequestDispatcher("xuekejianshetest.jsp").forward(req, resp);
			...
			List list = null;
			if ("0".equals(idtentity)) {
				list = xkjss.queryAll(currpage, everycount, userid, year, status,
						flag, inputvalue);
			} else if ("1".equals(idtentity)||"-1".equals(idtentity)||"-0".equals(idtentity)) {
				list = xkjss.queryByCollege(currpage, everycount, college, year,
						status, flag, inputvalue,yrank,school);
			} else if("2".equals(idtentity)){
				list = xkjss.queryAllColloge(currpage, everycount, year, status, flag, inputvalue,yrank,school);
			}
			...
			session.setAttribute("xkjsList", list);
			...
			if ("0".equals(idtentity)) {
				w.write("newiframexuekejianshe2.jsp," + currpage + "," + pagecount);
			} else if ("1".equals(idtentity)||"-1".equals(idtentity)||"-0".equals(idtentity)) {
				w.write("newiframexuekejianshe2-pass.jsp," + currpage + "," + pagecount);
			} else if ("2".equals(idtentity)) {
				w.write("newiframexuekejianshe2-pass-xiao.jsp," + currpage + "," + pagecount);
			}

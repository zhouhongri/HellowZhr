...
public class ShiYanShiJianSheServlet extends HttpServlet {
		...
		if (index != null) {
			if (flag == null) {
				if (rank != null) {
					...
					List<Map<String, String>> list = sysjss.queryScore(rank,
							authorseq, userid);
					w.write(list.get(0).get("yuan")+","+list.get(0).get("xiao"));
				} else {
					String zicd = "实验室建设";
					List rankList = sysjss.queryRank(userid, zicd);
					session.setAttribute("rankList", rankList);
					req.getRequestDispatcher("shiyanshijianshetest.jsp").forward(req, resp);
			...
			List list = null;
			if ("0".equals(idtentity)) {
				list = sysjss.queryAll(currpage, everycount, userid, year, status,
						flag, inputvalue);
			} else if ("1".equals(idtentity)||"-1".equals(idtentity)||"-0".equals(idtentity)) {
				list = sysjss.queryByCollege(currpage, everycount, college, year,
						status, flag, inputvalue,yrank,school);
			} else if("2".equals(idtentity)){
				list = sysjss.queryAllColloge(currpage, everycount, year, status, flag, inputvalue,yrank,school);
			}
			...
			session.setAttribute("sysjsList", list);
			...
			if ("0".equals(idtentity)) {
				w.write("newiframeshiyanshijianshe2.jsp," + currpage + "," + pagecount);
			} else if ("1".equals(idtentity)||"-1".equals(idtentity)||"-0".equals(idtentity)) {
				w.write("newiframeshiyanshijianshe2-pass.jsp," + currpage + "," + pagecount);
			}else if ("2".equals(idtentity)) {
				w.write("newiframeshiyanshijianshe2-pass-xiao.jsp," + currpage + "," + pagecount);
			}

...
public class KeChengJianSheServlet extends HttpServlet {
	private String everycount = "1"; // 每一页显示数据条数
		...
		if (index != null) {
			if (flag == null) {
				if (rank != null) {
					...
					List<Map<String, String>> list = kcjss.queryScore(rank,
							authorseq, userid);
					w.write(list.get(0).get("yuan")+","+list.get(0).get("xiao"));
				} else {
					String zicd = "课程建设";
					List rankList = kcjss.queryRank(userid, zicd);
					session.setAttribute("rankList", rankList);
					req.getRequestDispatcher("kechengjianshetest.jsp").forward(req, resp);
			...
			List list = null;
			if ("0".equals(idtentity)) {
				list = kcjss.queryAll(currpage, everycount, userid, year, status,
						flag, inputvalue);
			} else if ("1".equals(idtentity)||"-1".equals(idtentity)||"-0".equals(idtentity)) {
				list = kcjss.queryByCollege(currpage, everycount, college, year,
						status, flag, inputvalue,yrank,school);
			} else if("2".equals(idtentity)){
				list = kcjss.queryAllColloge(currpage, everycount, year, status, flag, inputvalue,yrank,school);
			}
			...
			session.setAttribute("kcjsList", list);
			...
			if ("0".equals(idtentity)) {
				w.write("newiframekechengjianshe2.jsp," + currpage + "," + pagecount);
			} else if ("1".equals(idtentity)||"-1".equals(idtentity)||"-0".equals(idtentity)) {
				w.write("newiframekechengjianshe2-pass.jsp," + currpage + "," + pagecount);
			}else if ("2".equals(idtentity)) {
				w.write("newiframekechengjianshe2-pass-xiao.jsp," + currpage + "," + pagecount);
			}

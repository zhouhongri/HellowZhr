...
public class JXYJXMServlet extends HttpServlet {
		...
		if (index != null) {
			if (flag == null) {
				if (rank != null) {
					...
					List<Map<String, String>> list = jxyjxms.queryScore(rank,
							authorseq, userid);
					w.write(list.get(0).get("yuan")+","+list.get(0).get("xiao"));
				} else {
					String zicd = "教学研究项目";
					List rankList = jxyjxms.queryRank(userid, zicd);
					session.setAttribute("rankList", rankList);
					req.getRequestDispatcher("jxyjxmtest.jsp").forward(req, resp);
			...
			List list = null;
			if ("0".equals(idtentity)) {
				list = jxyjxms.queryAll(currpage, everycount, userid, year, status,
						flag, inputvalue);
			} else if ("1".equals(idtentity)||"-1".equals(idtentity)||"-0".equals(idtentity)) {
				list = jxyjxms.queryByCollege(currpage, everycount, college, year,
						status, flag, inputvalue, yrank,school);
			} else if("2".equals(idtentity)){
				list = jxyjxms.queryAllColloge(currpage, everycount, year, status, flag, inputvalue, yrank,school);
			}
			...
			session.setAttribute("jxyjxmList", list);
			...
			if ("0".equals(idtentity)) {
				w.write("newiframejxyjxm2.jsp," + currpage + "," + pagecount);
			} else if ("1".equals(idtentity)||"-1".equals(idtentity)||"-0".equals(idtentity)) {
				w.write("newiframejxyjxm2-pass.jsp," + currpage + "," + pagecount);
			}else if ("2".equals(idtentity)) {
				w.write("newiframejxyjxm2-pass-xiao.jsp," + currpage + "," + pagecount);
			}

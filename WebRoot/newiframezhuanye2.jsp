     ...
     <table class="am-table am-table-striped am-table-hover table-main">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="table-author am-hide-sm-only">申办时间</th>
                                            <th class="table-type am-hide-sm-only">申办人</th>
                                            <th class="table-type am-hide-sm-only">申办人排序</th>
                                    ...
                                    <tbody>
                                    <c:if test="${fn:length(zyList)==0}"> 
										<tr align="center">
											<td colspan="16" style="color:red;">查询无结果</td>
										</tr>
										</c:if>
										<c:forEach items="${zyList}" var="a">
                                        <tr id="tr${a.id}">
                                            <td></td>
                                            <td>${a.name}</td>
...
<div class="modal fade" id="modal-container-834996" role="dialog" aria-hidden="true" aria-labelledby="myModalLabel">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
					 	<label class="col-sm-2 control-label" for="inputEmail3"><span style="color:red; font-size:20px;">*</span>专业名称</label>
						<div class="col-sm-10">
							<input class="form-control ipt"  type="text" name="name" id="zyname"/>
						</div>
					</div>
					<div class="form-group" >
					...
					<ol style="display: none;">
						<li style="display: none;">
							...
							<input style="display: none;" class="ipt" type="text" name="type" value="zhuanye" size="20" />
						</li>
						<li style="display: none;">
   ...
   //上传文件
	...
	function tiaozhuan(pagecount) {
	    ...
	    var url = "queryzl?currpage="+inputnum+"&year="+yearvalue+"&status="+ statusvalue+"&inputvalue=" + inputvalue;
	...
	function callback() {
		...
		var url="queryzy?rank="+encodeURI(encodeURI(rank))+"&authorseq="+authorseq+"&userid="+userid+"&index=1";
		xmlHttp.open("POST", url, true);
		xmlHttp.send(null)}
	}
	function callback2() {

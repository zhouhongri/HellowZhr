...
<script type="text/javascript">
	    ...
	    function danchuacprd(id,sltnameval){
  			...
  			bb1.href="xiaoexportzc?sltnameval="+encodeURI(encodeURI(sltnameval))+"&flag=xpzc"+"&jobname="+encodeURI(encodeURI("副教授"))+"&id="+id;
 		}
  		function danchuacscore(){
  			var bb2 = document.getElementById("bb2");
  			var college = document.getElementById("collegeipt").value;
  			bb2.href="xiaoexportscorezc?flag=xpzc"+"&jobname="+encodeURI(encodeURI("副教授"))+"&college="+encodeURI(encodeURI(college));
  		}
  		function danchuacsumscore(){
  			var bb3 = document.getElementById("bb3");
  			var college = document.getElementById("collegeipt").value;
  			bb3.href="xiaoexportsumscorezc?flag=xpzc"+"&jobname="+encodeURI(encodeURI("副教授"))+"&college="+encodeURI(encodeURI(college));
  		}
  		
		function prevPage(){
			var page = check("prevPage");
			if(page!="-1"){
				window.location.href = "queryxiaoauthorszc?currpage="+page;
			}
		}
	...
	function keyLogin(e){
  	...
  	function changecollege(rownum,college){
  		var id = document.getElementById("idc"+rownum);
  		id.href="queryxiaoauthorszc?currpage=1"+"&college="+encodeURI(encodeURI(college))+"&jobname=fjs";
  	}   		
  	</script>
...
<body>
	...
	<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
		<ul class="nav navbar-nav">
						<li class="dropdown">
						<a href="" class="dropdown-toggle" data-toggle="dropdown">院选择<strong class="caret"></strong></a>
							<ul class="dropdown-menu">
							<li>
									 <a href="javascript:void(0);" id="idc0" onclick="changecollege(0,'全部');">全部</a>
								</li>
								<c:forEach items="${listcollege}" var="ltce">
									<li>
										<a href="javascript:void(0);" id="idc${ltce.rownum}" onclick="changecollege(${ltce.rownum},'${ltce.college}');">${ltce.college}</a>
									</li>
								</c:forEach>
							</ul>
						</li>
		...
		<ul class="nav navbar-nav navbar-right">
			...
			<li><a href="javascript:void(0);" id="bb2"><input
					type="button" value="下载各项分数"
					onclick="danchuacscore();{
								  this.disabled = true;
								  var me = this;
			...
			<li>
			 <a href="javascript:void(0);" id="bb3"><input type="button"
					value="下载总分数"
					onclick="danchuacsumscore();{
								  this.disabled = true;
								  var me = this;
								  setTimeout(function() { me.disabled = false; }, 500);
								}">
	...
	<input type="text" value="${college}" id="collegeipt" style="display: none;"/>
	...
	<table class="table table-hover">
		...
		<c:forEach items="${listprd}" var="prd">
			<tr>
				...
				<td>${prd.sumscore}</td>
				<td><a href="javascript:void(0);" id="bb1${prd.id}"><input
						type="button" value="点击下载"
						onclick="danchuacprd(${prd.id},'${prd.authorname}');{
					  this.disabled = true;
					  var me = this;
					  setTimeout(function() { me.disabled = false; }, 500);
					 }">
	...
	<ul class="pagination" style="float:right;">
		...
		<li><a href="queryxiaoauthorszc?currpage=1">首页</a>
		</li>
		...
		<li><a href="queryxiaoauthorszc?currpage=${pagecount}">末页</a>
		</li>
	</ul>

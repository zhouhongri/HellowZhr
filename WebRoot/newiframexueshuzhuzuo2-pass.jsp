     ...
     <table class="am-table am-table-striped am-table-hover table-main">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="table-type am-hide-sm-only">出版日期</th>
                                    ...
                                    <tbody>
                                    <c:if test="${fn:length(xszzList)==0}"> 
										<tr align="center">
											<td colspan="15" style="color:red;">查询无结果</td>
										</tr>
										</c:if>
										<c:forEach items="${xszzList}" var="a">
                                        <tr id="tr${a.id}">
                                            <td></td>
                                            <td>${a.name}</td>
 ...
 <script src="assets/js/jquery.min.js"></script>
   ...
   $("#mainFrame",parent.document).css("height",tableHeight);
	...
	function tiaozhuan(pagecount) {
	    ...
	    var url = "queryxxzz?currpage="+inputnum+"&year="+yearvalue+"&status="+ statusvalue+"&inputvalue=" + inputvalue+"&yrank="+yrankvalue;
	...
	function callback() {
		...
		var url="queryxszz?rank="+encodeURI(encodeURI(rank))+"&authorseq="+authorseq+"&userid="+userid+"&index=1";
		xmlHttp.open("POST", url, true);
		xmlHttp.send(null)}
	}
	function callback2() {

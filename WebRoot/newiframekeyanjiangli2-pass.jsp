     ...
     <table class="am-table am-table-striped am-table-hover table-main">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="table-title">科研名称</th>
                                            <th class="table-type am-hide-sm-only">获奖时间</th>
                                            <th class="table-type am-hide-sm-only">获奖者</th>
                                            <th class="table-type am-hide-sm-only">获奖者排序</th>
                                    ...
                                    <tbody>
                                    <c:if test="${fn:length(kyjlList)==0}"> 
										<tr align="center">
											<td colspan="15" style="color:red;">查询无结果</td>
										</tr>
										</c:if>
										<c:forEach items="${kyjlList}" var="a">
                                        <tr id="tr${a.id}">
                                            <td></td>
                                            <td>${a.name}</td>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
					 	<label class="col-sm-2 control-label" for="inputEmail3"><span style="color:red; font-size:20px;">*</span>科研名称</label>
						<div class="col-sm-10">
 ...
 <script src="assets/js/jquery.min.js"></script>
   ...
   $("#mainFrame",parent.document).css("height",tableHeight);
	...
	function tiaozhuan(pagecount) {
	    ...
	    var url = "querykyjl?currpage="+inputnum+"&year="+yearvalue+"&status="+ statusvalue+"&inputvalue=" + inputvalue+"&yrank="+yrankvalue;
	...
	function callback() {
		...
		var url="querykyjl?rank="+encodeURI(encodeURI(rank))+"&authorseq="+authorseq+"&userid="+userid+"&index=1";
		xmlHttp.open("POST", url, true);
		xmlHttp.send(null)}
	}
	function callback2() {

     ...
     <table class="am-table am-table-striped am-table-hover table-main">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="table-title">实验室名</th>
                                            <th class="table-type am-hide-sm-only">申请时间</th>
                                            <th class="table-type am-hide-sm-only">负责人</th>
                                            <th class="table-type am-hide-sm-only">负责人排序</th>
                                    ...
                                    <tbody>
                                    <c:if test="${fn:length(sysjsList)==0}"> 
										<tr align="center">
											<td colspan="15" style="color:red;">查询无结果</td>
										</tr>
										</c:if>
										<c:forEach items="${sysjsList}" var="a">
                                        <tr id="tr${a.id}">
                                            <td></td>
                                            <td>${a.name}</td>
				...
				<div class="modal-body">
					...
					<ol style="display: none;">
						<li style="display: none;">
							...
							<input style="display: none;" class="ipt" type="text" name="type" value="shiyanshijianshe" size="20" />
						</li>
						<li style="display: none;">
 ...
 <script src="assets/js/jquery.min.js"></script>
   ...
   $("#mainFrame",parent.document).css("height",tableHeight);
	...
	function tiaozhuan(pagecount) {
	    ...
	    var url = "querysysjs?currpage="+inputnum+"&year="+yearvalue+"&status="+ statusvalue+"&inputvalue=" + inputvalue+"&yrank="+yrankvalue;
	...
	function callback() {
		...
		var url="querysysjs?rank="+encodeURI(encodeURI(rank))+"&authorseq="+authorseq+"&userid="+userid+"&index=1";
		xmlHttp.open("POST", url, true);
		xmlHttp.send(null)}
	}
	function callback2() {

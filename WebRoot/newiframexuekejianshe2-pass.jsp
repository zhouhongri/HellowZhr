...
<p id="userid" style="display: none;"><%=session.getAttribute("userid") %></p>
     <table class="am-table am-table-striped am-table-hover table-main">
                                    ...
                                    <tbody>
                                    <c:if test="${fn:length(xkjsList)==0}"> 
										<tr align="center">
											<td colspan="15" style="color:red;">查询无结果</td>
										</tr>
										</c:if>
										<c:forEach items="${xkjsList}" var="a">
                                        <tr id="tr${a.id}">
                                            <td></td>
                                            <td>${a.name}</td>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
					 	<label class="col-sm-2 control-label" for="inputEmail3"><span style="color:red; font-size:20px;">*</span>学科名称</label>
						<div class="col-sm-10">
...
function tiaozhuan(pagecount) {
	    ...
	    var url = "queryxkjs?currpage="+inputnum+"&year="+yearvalue+"&status="+ statusvalue+"&inputvalue=" + inputvalue+"&yrank="+yrankvalue;
	...
	function callback() {
		...
		var url="queryxkjs?rank="+encodeURI(encodeURI(rank))+"&authorseq="+authorseq+"&userid="+userid+"&index=1";
		xmlHttp.open("POST", url, true);
		xmlHttp.send(null)}
	}
	function callback2() {
